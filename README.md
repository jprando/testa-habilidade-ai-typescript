...
async function processWithLimit<T, R>(
    ...
    // Ponteiro compartilhado entre os workers
    let currentIndex = 0;

    // A função worker é um loop assíncrono.
    // Ela pega o próximo item disponível, processa, salva no índice correto e repete.
    const worker = async () => {
        while (currentIndex < items.length) {
            // Captura o índice atual e já incrementa para o próximo worker de forma atômica
            const index = currentIndex++;

            // Executa a tarefa e salva o resultado no slot correspondente
            results[index] = await asyncFn(items[index], index);
        }
    };

    // Cria a quantidade exata de workers permitida pelo limite
    // (ou menos, se a lista for menor que o limite)
    const concurrency = Math.min(limit, items.length);
    const workers = Array.from({ length: concurrency }, () => worker());

    // Aguarda todos os workers finalizarem seu trabalho
